...
export const makeMessagesSocket = (config: SocketConfig) => {
	...
	/**
	 * [PATCH-007] Cria o conteúdo do `<biz>` binary node que sinaliza ao
	 * WhatsApp que o stanza carrega uma mensagem "rica". Tags possíveis:
	 *   - `<list type="product_list" v="2">`       para listMessage
	 *   - `<interactive type="native_flow" v="1">` para buttonsMessage / nativeFlow
	 * Sem este node no stanza, botões não são renderizados no destinatário
	 * mesmo com o PATCH-006 (envelope documentWithCaption) aplicado.
	 */
	...
	const relayMessage = async (
		...
		await authState.keys.transaction(async () => {
			...
			// [PATCH-007] Anexa `<biz>` binary node com interactive/list info.
			// Sem este node, o WhatsApp não reconhece a mensagem como rica e
			// mesmo envelopada em documentWithCaption (PATCH-006) não é processada
			// como botões no destinatário. Lê DENTRO do envelope pra decidir a tag.
			{
				const innerMessage = message.documentWithCaptionMessage?.message || message
				const bizContent = createButtonNode(innerMessage)
				if (bizContent) {
					;(stanza.content as BinaryNode[]).push({
						tag: 'biz',
						attrs: {},
					...
					logger.debug({ jid }, '[PATCH-007] adding biz node for rich message')
				}
			}

			// [PATCH-005] Anexa <reporting> binary node pras mensagens que qualificam
			// (tudo exceto reaction/poll-update). Sem este node, mensagens ricas são
			// aceitas pelo WhatsApp mas NÃO entregues ao destinatário — sintoma:
			// "✓" local mas nada chega. Portado do whaileys.
			if (
				!isRetryResend &&
				...
				try {
					const patchedForReport = await patchMessageBeforeSending(message)
					// Quando patchMessageBeforeSending devolve array (DM per-JID), o primeiro
					// item é uma IMessage completa.
					const encForReport = Array.isArray(patchedForReport)
						? encodeWAMessage(patchedForReport[0]!)
						: encodeWAMessage(patchedForReport)
					const reportingKey: proto.IMessageKey = {
						id: msgId,
						fromMe: true,
						remoteJid: destinationJid
					}
					const reportingNode = await getMessageReportingToken(encForReport, message, reportingKey)
					if (reportingNode) {
						;(stanza.content as BinaryNode[]).push(reportingNode)
						logger.debug({ jid, msgId }, '[PATCH-005] reporting token attached')
					}
				} catch (err) {
					logger.warn({ jid, err }, '[PATCH-005] failed to attach reporting token')
				}
			}
